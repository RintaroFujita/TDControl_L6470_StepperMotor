...
void loop() {
  if (Serial.available() > 0) {
    ...
    if (flag == 1) {
      rotateMotorMicrostep(1, 200);//モーターが1回転し、回転速度が200で回る
    } else {
      stopMotor();  // モーターを停止
    }
  }
...
void L6470_send(unsigned char value) {
  //unsigned charは1バイト分のメモリを占めるデータ型(バイト)0から255までの8ビットの値
...
void rotateMotorMicrostep(float kaiten, int speed) {
  // 回転方向と速度
  if (kaiten > 0) {
    L6470_send(0x50);//もし、kaitenが0より大きいと回る。逆回転をしたかったら0x51にすると逆回転する
  } 
